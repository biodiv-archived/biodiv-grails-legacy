    ...
    def getJson(List<Long> ids,Map<String,Map<String,String>> customFieldMapToObjectId) {
        ...
        SELECT obs.id,
          ...
          String queryForObservationLike="""
           ...
           CASE  when su.icon is not null then su.icon else su.profile_pic END as icon ,
           r.rater_id as userid
           from rating as r inner join suser  as su on r.rater_id=su.id inner join  rating_link as rl on r.id=rl.rating_id
            where rl.rating_ref in ( """+sids+""" ) and rl.type='observation'
            """;

          def queryForObservationLikeResult=sql.rows(queryForObservationLike);
          println "&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&"
          println queryForObservationLikeResult.getClass()

          Map<String,List<Map<String,Object>>> observationLike=new Map<String,List<Map<String,Object>>>();

          println "****************************"
          println queryForObservationLikeResult;
          println "*****************************"
