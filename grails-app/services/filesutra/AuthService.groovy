...
class AuthService {
    ...
    Access googleLogin(emailId, accessInfo) {
        ...
        if (!access) {
            access = new Access(type: "GOOGLE", emailId: emailId)
        } else{
    ...
    Access facebookLogin(emailId, accessInfo) {
        ...
        if (!access) {
            access = new Access(type: "FACEBOOK", emailId: emailId)
        }
        access.accessInfo = Utils.jsonToString(accessInfo)
        access.save(flush: true, failOnError: true)
        return access
    ...
    Access flickrLogin(emailId, accessInfo) {
        ...
        if (!access) {
            access = new Access(type: "FLICKR", emailId: emailId)
        }
        access.accessInfo = Utils.jsonToString(accessInfo)
        access.save(flush: true, failOnError: true)
        return access
     ...
     Access picasaLogin(emailId, accessInfo) {
        ...
        if (!access) {
            access = new Access(type: "PICASA", emailId: emailId)
        }
        access.accessInfo = Utils.jsonToString(accessInfo)
        access.save(flush: true, failOnError: true)
        return access
    ...
    Access dropboxLogin(emailId, accessInfo) {
        ...
        if (!access) {
            access = new Access(type: "DROPBOX", emailId: emailId)
        }
        access.accessInfo = Utils.jsonToString(accessInfo)
        access.save(flush: true, failOnError: true)
        return access
