...
class AdminController {
	...
	def loadData = {
		...
		try {
			...
			noOfInsertions += dataLoader.uploadSpreadsheet(grailsApplication.config.speciesPortal.data.rootDir+"/speciespages/speciespageszip/mango/mango/MangoMangifera_indica_prabha_v4 (copy).xlsx", 0, 0, 1, 4);
			...
			noOfInsertions += dataLoader.uploadSpreadsheet(grailsApplication.config.speciesPortal.data.rootDir+"/speciespages/speciespageszip/grey_falcolin/GreyFrancolin_v4.xlsx", 0, 0, 1, 4);
			...
			noOfInsertions += dataLoader.uploadNewSpreadsheet(grailsApplication.config.speciesPortal.data.rootDir+"/speciespages/Rufous Woodpecker/RufousWoodepecker_v4_1.xlsm");
			...
			noOfInsertions += dataLoader.uploadNewSpreadsheet(grailsApplication.config.speciesPortal.data.rootDir+"/speciespages/Eurasian Curlew/EurasianCurlew_v4_2.xlsm");
			...
			noOfInsertions += dataLoader.uploadMappedSpreadsheet(grailsApplication.config.speciesPortal.data.rootDir+"/speciespages/Dung_beetle_Species_pages_IBP_v13.xlsx", grailsApplication.config.speciesPortal.data.rootDir+"/mappings/dungbeetles_mapping.xlsx", 0, 0, 0, 0);

			grailsApplication.config.speciesPortal.images.uploadDir = grailsApplication.config.speciesPortal.data.rootDir+"/speciespages/images";
			noOfInsertions += dataLoader.uploadMappedSpreadsheet(grailsApplication.config.speciesPortal.data.rootDir+"/speciespages/Trees_descriptives_prabha_final_6.xlsx", grailsApplication.config.speciesPortal.data.rootDir+"/mappings/ifp_tree_mapping_v2.xlsx", 0, 0, 0, 2);

			//grailsApplication.config.speciesPortal.images.uploadDir = grailsApplication.config.speciesPortal.data.rootDir+"/speciespages/images";
			noOfInsertions += dataLoader.uploadMappedSpreadsheet(grailsApplication.config.speciesPortal.data.rootDir+"/speciespages/Bats/WG_bats_account_01Nov11_sanjayMolur.xls", grailsApplication.config.speciesPortal.data.rootDir+"/mappings/WG_bats_account_01Nov11_sanjayMolurspecies_mapping_v2.xlsx", 0, 0, 0, 0);
			...
			noOfInsertions += dataLoader.uploadKeyStoneData("jdbc:mysql://localhost:3306/ezpz", "sravanthi", "sra123", mappingFile, 0, 0);
	...
	def loadNames = {
		...
		try {
			...
			e.printStackTrace();
			flash.message = "Error updating flowers of india names : ${e.getMessage()}";
		}
		...
		try {
			...
			e.printStackTrace();
			flash.message = "Error updating eflora names ${e.getMessage()}";
		}
		...
		try {
			...
			e.printStackTrace();
			flash.message = "Error updating fishbase names : ${e.getMessage()}"
		}
		...
		new File(gbifTaxaFile).splitEachLine("\\t") { fields ->
			...
			if(i%500 == 0) {
				...
				registry.eachWithIndex { e, index ->
					e.save();
					e.errors.each { println it }
					if(index%100 == 0) {
						cleanUpGorm();
		...
		sql.eachRow("select * from birdspecies_list") { row ->
			...
			new Node(taxon1, "data", name)
			taxonEntries.add(taxon1);
			registry = converter.getClassifications(taxonEntries, name);
			registry.each { e ->
				e.save(flush:true);
				e.errors.each { println it }
			}
