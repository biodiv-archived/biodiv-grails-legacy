    ...
    <div class="observation_story_body ${showFeatured?'toggle_story':''}" style=" ${showFeatured?'display:none;':''}">
        <g:if test="${showTitleDetail}">
            <div class="prop">
                ...
                <div class="value">
                        <a href="${uGroup.createLink(controller:'dataset', action: 'show', id:datasetInstance.id, 'userGroup':userGroup, 'userGroupWebaddress':userGroupWebaddress)}"><b>${datasetInstance.title} </b></a>
                </div>
            </div>
        </g:if>
