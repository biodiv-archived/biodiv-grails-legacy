...
<div class="trait  ${filterable==false?'':'filterable'}" data-id="${traitInstance.id}">
	<a href="${uGroup.createLink(action:'show', controller:'trait', id:traitInstance.id)}">
        <h6>${traitInstance.name}${traitInstance.units?"("+traitInstance.units.value()+")":''}
    ...
    <g:if test="${editable}">
    <g:if test="${ifOwns || traitInstance.isParticipatory || fromSpeciesShow}">
