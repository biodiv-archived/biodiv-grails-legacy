...
<div class="block-tagadelic ">
	...
	<form id="advSearchForm" method="get" title="Search"
		...
		<label class="control-label" for="name">Title</label> <input
			id="title" data-provide="typeahead" type="text"
			class="input-block-level" name="aq.name"
			placeholder="Search by File title" value="${params.name}" /> <label
	...
	$('#advSearchForm :input:not(input[type=hidden])').each(function(index, ele) {
		...
		$(this).typeahead({
			source: function (query, process) {
	        	return $.get("${uGroup.createLink(action:'terms', controller:'UFile') }"+'?field='+field, { term: query }, function (data) {
	            	return process(data);
	        	});
    		}
		});
