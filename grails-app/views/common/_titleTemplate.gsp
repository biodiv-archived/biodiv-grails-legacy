...
<title>${title}<g:if test="${params.webaddress}"> | ${userGroupInstance.name} </g:if><g:if test="${params.action.equals('show')}"> | ${params.controller.capitalize()} </g:if> | ${Utils.getDomainName(request)}</title>
