...
$(document).ready(function(){
	$("#speciesIdentifiedGroupFilter").button();
	$('#speciesIdentifiedGroupFilter button[value="${params.sGroup}"]').addClass('active');
        $('#speciesIdentifiedGroupFilter button').tooltip({placement:'top'});
	
	
	$("#habitatIdentifiedFilter").button();
	$('#habitatIdentifiedFilter button[value="${params.habitat}"]').addClass('active');
	$('#habitatIdentifiedFilter button').tooltip({placement:'bottom'});
		
});

</script>
<div id="speciesIdentifiedGroupFilter" data-toggle="buttons-radio">
	...
	<g:each in="${SpeciesGroup.list() }" var="sGroup" status="i">
		<g:if test="${sGroup != othersGroup }">
			<button class="btn species_groups_sprites ${sGroup.iconClass()}"
				id="${"identifiedGroup_" + sGroup.id}" value="${sGroup.id}" data-identified="identified"
