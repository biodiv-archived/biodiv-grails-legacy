...
<div class="story-actions clearfix" style="width: 100%;">
    ...
    <div class="span4 nav clearfix" style="margin-left:20px;margin-bottom:3px;">
        ...
        if(pos>=0 && (prevObservationId || nextObservationId)) 
            navParams['pos'] = pos-1; 
        %>
        <a class="pull-left btn  btn-mini ${prevId?:'disabled'}" href="${uGroup.createLink(navParams.clone())}"><i class="icon-backward"></i><g:message code="msg.Prev" />Prev</a>

        <% navParams['id'] = nextId; 
        if(pos>=0 && (prevObservationId || nextObservationId))
            navParams['pos'] = pos+1; 
        %>
        <a class="pull-right  btn btn-mini ${nextId?:'disabled'}"  href="${uGroup.createLink(navParams.clone())}"><g:message code="msg.Next" />Next<i style="margin-right: 0px; margin-left: 3px;" class="icon-forward"></i></a>
