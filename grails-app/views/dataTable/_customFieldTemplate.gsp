...
<div class="control-group customField"  style="margin-top:5px;">
	<g:if test="${!hideLabel}">
      ...
      <%
		...
		def options =  customFieldInstance.options?customFieldInstance.options.split(','):[];
      ...
      <g:if test="${options}">
      	<g:if test="${customFieldInstance.allowedMultiple}">
      		<select class="${CustomField.PREFIX + 'multiselectcombo' + mandatoryFieldClass}" multiple="multiple" title="${customFieldInstance.notes}" placeholder="${customFieldInstance.notes}" name="${CustomField.PREFIX + customFieldInstance.name}" >
   			<g:each in="${options}">
      	...
      	<g:else>
      		<select class="${'combobox' + mandatoryFieldClass}" name="${CustomField.PREFIX + customFieldInstance.name}" placeholder="${customFieldInstance.notes}" title="${customFieldInstance.notes}" >
      			<option></option>
    			<g:each in="${options}">
      				<g:if test="${cValue == it}">
        				<option selected="selected" value="${it}"> ${it}</option>
...
<%--		<b style="margin-left:3%;">Or</b> <input style="float:right; width:60%;" type="text" name="${CustomField.PREFIX + customFieldInstance.name}" placeholder="Add new value" class="input-block-level"/>--%>
	  ...
	  <g:else>
	  	<g:if test="${CustomField.DataType.PARAGRAPH_TEXT == customFieldInstance.dataType}">
	  		<textarea style="resize: vertical;" class="${'input-block-level' + mandatoryFieldClass}" placeholder="${customFieldInstance.notes}" title="${customFieldInstance.notes}" name="${CustomField.PREFIX + customFieldInstance.name}">${cValue}</textarea>
        </g:if>
        <g:elseif test="${CustomField.DataType.DATE == customFieldInstance.dataType}">
			...
			%>
        	<input name="${CustomField.PREFIX + customFieldInstance.name}" placeholder="${customFieldInstance.notes}" title="${customFieldInstance.notes}" type="text" class="${'date input-block-level' + mandatoryFieldClass}"
        			value="${cValue?.format('dd/MM/yyyy')}"/>
        </g:elseif>
        <g:else>	
          <div class="textbox">
              <input type="text" placeholder="${customFieldInstance.notes}" title="${customFieldInstance.notes}" name="${CustomField.PREFIX + customFieldInstance.name}" value="${cValue}" class="${'input-block-level ' + (isNumber?'CustomField_number':'') + mandatoryFieldClass}"/>
          </div>
        </g:else>  
      </g:else>    
      </div>
