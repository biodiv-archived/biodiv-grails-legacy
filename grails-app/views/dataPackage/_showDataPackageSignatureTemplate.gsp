        ...
        style="display: table; width:100%;height: ${showDetails ? '50px;':'40px;'}">
            ...
            <a target="_blank"
                ...
                <span class="ellipsis  ${showDetails ? 'multiline' : ''}" style="display: block;text-align:left;${showDetails ? 'width:auto' : 'width:200px'};"
                    title="${instance.title}"><b>DataPackage : </b> ${instance.title} </span> 
            </a>
            <g:if test="${instance && instance.id}">
            <div class="footer-item" title="${g.message(code:'showdataPackage.title.datasetCount')}">
                <i class="icon-book"></i>
                ${Dataset.countByDataPackage(instance)}
            </div>
           </g:if>
    </div>
</div>
