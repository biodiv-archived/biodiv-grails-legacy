...
<div id="childList">
    <g:each var="uFile" in="${uFiles}" status="i">
     ...
     <!-- Render the addFile template (_addFile.gsp) here -->
        <g:render template='/addFile' model="['uFile':uFile,'i':i,'hidden':false]"/>
...
 <g:render template='/addFile' model="['uFile':null,'i':'_clone','hidden':true]"/>
<input type="button" value="Add File" onclick="addUFile();" />      