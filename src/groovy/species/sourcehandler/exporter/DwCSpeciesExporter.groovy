...
def exportSpecieData(directory, DownloadLog dl) {
		...
		String folderPath = dir +f +folderName+ f+ folderName
...
//			"rightsHolder"
	...
	public def exportSpecie(list_of_species, reqUser, dl_id , params_filterUrl) {
		...
		exportDescriptionMedia(list_of_species)
	...
	public def exportTaxonSpecie( list_of_species, reqUser, dl_id , params_filterUrl) {
		...
		taxonRow[2] = (!specie?.taxonConcept?.name) ? "" : specie?.taxonConcept?.name; 
			...
			ibpHierarchy?.hierarchies?.each{  taxon ->
	...
	public def exportDescriptionMedia(list_of_species){
